...
uint32_t red_execuation_monitor;
...
void vOrangeLedController(void *Parameter)
{
	...
	while(1)
		...
		{
			resume_monitor++;
			if(resume_monitor>=100)
			{
				osThreadResume(redID);
...
void vBlueLedController(void *Parameter)
{
	...
	while(1)
		...
		red_execuation_monitor++;

		if(red_execuation_monitor ==49)
		{
			killed=true;

			red_execuation_monitor=0;

			osThreadTerminate(redID);
